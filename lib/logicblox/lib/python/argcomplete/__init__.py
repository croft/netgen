...
import os, sys, argparse, contextlib, subprocess, locale, itertools
import traceback
...
def autocomplete(argument_parser, always_complete_options=True, exit_method=os._exit, output_stream=None):
    ...
    def is_final_arg():
        if comp_words and comp_words[-1] in ['-h', '--help', '--version']:
            return True
    ...
    for parser in active_parsers[-1:]:
        ...
        if (len(cword_prefix) == 0 or cword_prefix[0] not in parser.prefix_chars):
            for active_action in parser.active_actions:
                # debug('----------- active action', active_action)
                ...
                if len(active_action.option_strings) > 0: # only for optionals
                    if not action_is_satisfied(active_action):
                        ...
                        completions = []
                        unsatisfied_optional_arg = True
                    else:
                        debug("Action is satisfied")
        ...
        if (unsatisfied_optional_arg or not parser.parsed_positional):
            for action in parser._actions:
                debug("--Examining action", action)
                ## don't auto-complete if argument has help=SUPPRESS
    ...
    # debug('cword_prequote', cword_prequote)
    # debug('cword_prefix', cword_prefix)

    if cword_prequote == '':
        ## LB: we comment this out to make completion with colons work.
        ## this is strange since the very purpose of the code below is to make that
        ## same thing work... :|
