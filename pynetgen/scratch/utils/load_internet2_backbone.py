...
def load_ntf(rtr_name):
    f = TF(1)
    f.load_object_from_file("../data_set/Internet2/%s.tf"%rtr_name)
    f.activate_exact_match_hash(range(5,8))
    return f

def load_internet2_backbone_ttf():
    f = TF(1)
    f.load_object_from_file("../data_set/Internet2/backbone_topology.tf")
    return f

def load_internet2_backbone_port_to_id_map():
    f = open("../data_set/Internet2/port_map.txt",'r')
    id_to_name = {}
    map = {}
    rtr = ""
    cs = juniperRouter(1)
