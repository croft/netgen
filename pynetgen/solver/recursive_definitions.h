...
class Cyclicity : public recursive_definition {
       public:
        ...
        expr auxilary_def() const { return ctx.bool_val(true); }
        expr encode_null(int pc) const { return cycle(VALUE(0), VALUE(pc)) == VALUE(0); }
};
...
// Modified_Functionality(context & ctx_i, func_decl& rho_i,func_decl& delta_i, Automata& a1_i, set<int>& abstract_nodes_i, expr & delta_expri,
// expr_vector & delta_varsi)
// :ctx(ctx_i), rho(rho_i), delta(delta_i), a1(a1_i), abstract_nodes(abstract_nodes_i), delta_expr(delta_expri), delta_vars(delta_varsi)
        ...
        //                                 delta_str << "( ite ( and ( = state " << state << " ) ( = node " << node << " )) "
        //                                           << a1.transitions[make_pair(state, node)] << "  \n ";
        //                                 count++;
        //                         }
        //                         // else
        ...
        //                 for (auto node : abstract_nodes) {
        //                         if (a1.transitions.find(make_pair(state, node)) != a1.transitions.end())
        //                                 delta_expr = ite(state_expr == ctx.int_val(state) && node_expr == ctx.int_val(node),
        //                                                  ctx.int_val(a1.transitions[make_pair(state, node)]), delta_expr);
        //                         // else
        //                         // 	delta_expr = ctx.int_val(0);
        //                 }
        //         }
...
#if STATE == EQSTATE
                        query = query &&
                                delta(VALUE(tran_it->first.first), eqstate(VALUE(tran_it->first.second)) /*this is changing*/) ==
                                        VALUE(tran_it->second);
#elif STATE == WOEQSTATE
...
// }
        ...
        expr encode_null(int pc) const { return rho(VALUE(0), VALUE(pc)) == VALUE(0); }
};
...
class Compute_Dest : public recursive_definition {
       public:
        ...
        expr encode_null(int pc) const { return dest(VALUE(0), VALUE(pc)) == VALUE(0); }
};
