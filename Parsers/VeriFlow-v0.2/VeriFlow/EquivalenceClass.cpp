...
EquivalenceClass::EquivalenceClass(const uint64_t* lb, const uint64_t* ub)
{
	for(int i = 0; i < ALL_FIELD_INDEX_END_MARKER; i++)
	{
		if(lb[i] > ub[i])
		{
			...
			fprintf(stderr, "\tLower bound: %llu\n", lb[i]);
			fprintf(stderr, "\tUpper bound: %llu\n", ub[i]);
...
{
	char buffer[1024];
	sprintf(buffer, "[EquivalenceClass] dl_src (%llu-%s, %llu-%s), dl_dst (%llu-%s, %llu-%s)",
	...
	sprintf(buffer, "nw_src (%llu-%s, %llu-%s), nw_dst (%llu-%s, %llu-%s)",
	...
	for(int i = 0; i < ALL_FIELD_INDEX_END_MARKER; i++)
	{
		sprintf(buffer, "Field %d (%llu, %llu)", i, this->lowerBound[i], this->upperBound[i]);
		retVal += buffer;
