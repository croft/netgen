...
def reverse(fsa):
    ...
    return fsa.create(states + [newInitial], alpha, map(lambda (s0, s1, l):(s1, s0, l), transitions) + map(lambda s1, s0=newInitial:(s0, s1, EPSILON), finals), newInitial, [initial])
...
def compileRE(s, alphabet=None, **options):
    import string
    if not options.get('multichar'):
        s = string.replace(s, ' ', '')
    fsa, index = compileREExpr(s + ')', 0, alphabet, options)
    if index < len(s):
        raise 'extra ' + `')'`
    fsa.label = str(s)
    return fsa.minimized()

def compileREExpr(str, index, alphabet, options):
    fsa = None
    while index < len(str) and str[index] != ')':
        fsa2, index = compileConjunction(str, index, alphabet, options)
        if  str[index] == '|': index = index + 1
        fsa = (fsa and union(fsa, fsa2)) or fsa2
    return (fsa or EMPTY_STRING_FSA), index

def compileConjunction(str, index, alphabet, options):
    fsa = UNIVERSAL_FSA
    while str[index] not in ')|':
        conjunct, index = compileSequence(str, index, alphabet, options)
        if  str[index] == '&': index = index + 1
        fsa = intersection(fsa, conjunct)
    return fsa, index

def compileSequence(str, index, alphabet, options):
    fsa = EMPTY_STRING_FSA
    while str[index] not in ')|&':
        fsa2, index = compileItem(str, index, alphabet, options)
        fsa = concatenation(fsa, fsa2)
    return fsa, index

def compileItem(str, index, alphabet, options):
    c , index = str[index], index + 1
    while c == ' ':
        c, index = str[index], index + 1

    if c == '(':
        fsa, index = compileREExpr(str, index, alphabet, options)
        assert str[index] == ')'
        index = index + 1
    elif c == '.':
        fsa = singleton(ANY, alphabet=alphabet)
    elif c == '~':
        fsa, index = compileItem(str, index, alphabet, options)
        fsa = complement(fsa)
    else:
        if alphabet is not None:
            while c not in alphabet:
                # match longest substring
                lookahead = 1
                while index + lookahead < len(str):
                    if c + str[index+lookahead] in alphabet:
                        lookahead += 1
                    else:
                        break

                c += str[index:index+lookahead]
                index += lookahead
